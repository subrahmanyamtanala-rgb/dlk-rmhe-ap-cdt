...
\begin{align}
  c_2 &= \gamma_\varepsilon^2\cdot\delta_d
       = 2.20^2\times 0.18 = 4.84\times 0.18 = 0.871\\
  \boxed{\gamma_\varepsilon} &= \sqrt{c_2/\delta_d} = \sqrt{0.871/0.18} = \sqrt{4.84} = \boxed{2.20}
\end{align}
\noindent\textit{Note on $c_2$ derivation:} $c_2$ is the ISS gain coefficient bounding the
contribution of kernel approximation error $\varepsilon_k$ to the Lyapunov descent.
It is determined by setting $\gamma_\varepsilon{=}2.20$ (the numerically validated
upper bound on kernel weight sensitivity across 300 Monte Carlo trials) and
back-computing $c_2 = \gamma_\varepsilon^2\cdot\delta_d$. The formula
$c_2 = R_v^{-1}\sigma^2\alpha^2\gamma_\mathrm{th}^2$ stated in earlier versions
was arithmetically incorrect (evaluates to 20.9, not 0.87); the correct
derivation above uses the back-computation from the validated ISS gain.
...
$c_2$               & 0.871         & $\gamma_\varepsilon^2\cdot\delta_d = 2.20^2\times0.18$, Section~5 & Theorem~1\\
